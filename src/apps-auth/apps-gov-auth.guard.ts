...
@Injectable()
export class AppsGovAuthGuard implements CanActivate {
  constructor(
    private reflector: Reflector,
    private readonly appsAuthService: AppsAuthService
  ) {}
  /**
   *
   */
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    console.log(token);
    const user = await this.appsAuthService.verifyLoginUser(token);
    console.log(user);
